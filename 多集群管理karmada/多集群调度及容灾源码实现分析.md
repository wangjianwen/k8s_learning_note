...
如图所示：
- 用户创建crd: PropagationPolicy
- controller-manager中ResourceDetector控制器监听到PropagationPolicy的创建事件，根据PropagationPolicy的定义创建crd: ResourceBindings。注意：此时ResourceBindings的spec.Cluster字段为空，即还没有把工作负载分配到目标集群。
- scheduler会监听到ResourceBindings的创建事件，根据ResourceBindings中spec.Placement的定义，按照一定的算法确定工作负载将调度到哪些目标集群及确定对应的副本数
- controller-manager中ResourceDetector控制器为每个目标集群，按照ResourceBindings中工作负载的定义创建crd: Work
- controller-manager中execution控制器根据work的定义，调用目标集群api，创建真正工作负载
...
### 当某个集群故障时其工作负载从故障集群迁移到健康集群

#### cluster控制器

会不断地监听所有集群的健康状态，当集群不健康时会执行如下操作：
